...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await Promise.resolve().then(() => require("./fileFromPath-hhMyKjdO.js"));
...
function levenshtein(a2, b2) {
  const matrix = [];
  for (let i2 = 0; i2 <= b2.length; i2++) matrix[i2] = [i2];
  for (let j2 = 0; j2 <= a2.length; j2++) matrix[0][j2] = j2;
...
function findByName(query, items, maxDistance = 3) {
...
{
  "action": "move_note" | "create_folder" | "delete_folder" | "rename_note" | "rename_folder" | "list" | "question",
  "noteName": "name of note mentioned (if any)",
  "folderName": "name of folder mentioned (if any)",
  "newName": "new name for rename actions (if any)",
...
async function resolveIntent(intent) {
  ...
  switch (intent.action) {
    case "move_note": {
      if (!intent.noteName) {
        return { resolved: false, error: "No note name specified" };
      }
      if (!intent.folderName) {
        return { resolved: false, error: "No folder name specified" };
      }
      const note = findByName(intent.noteName, notes2);
      const folder = findByName(intent.folderName, folders2);
      if (!note) {
        return { resolved: false, error: `Note "${intent.noteName}" not found` };
      }
      if (!folder) {
        return { resolved: false, error: `Folder "${intent.folderName}" not found` };
      }
      console.log(`[Intent Resolver] Resolved: note "${note.title}" (${note.id}) → folder "${folder.name}" (${folder.id})`);
      return {
        resolved: true,
        action: {
          ...
          execute: async () => {
            await noteRepository.update(note.id, { folderId: folder.id });
            return { success: true, message: `Moved "${note.title}" to "${folder.name}"` };
          }
        }
      };
    }
    case "create_folder": {
      ...
      const existing = findByName(intent.folderName, folders2);
      ...
      return {
        ...
        action: {
          ...
          execute: async () => {
            await folderRepository.create({ name: intent.folderName });
            return { success: true, message: `Created folder "${intent.folderName}"` };
          }
        }
      };
    }
    case "delete_folder": {
      ...
      return {
        ...
        action: {
          ...
          execute: async () => {
            const folderNotes = await noteRepository.findByFolderId(folder.id);
            for (const n2 of folderNotes) {
              await noteRepository.update(n2.id, { folderId: null });
            }
            await folderRepository.delete(folder.id);
            return { success: true, message: `Deleted folder "${folder.name}"` };
          }
        }
      };
    }
    case "list": {
      return {
        ...
        action: {
          ...
          arguments: {},
          execute: async () => {
            const noteList = notes2.map((n2) => n2.title).join(", ");
            const folderList = folders2.map((f2) => f2.name).join(", ");
            return {
              success: true,
              message: `Notes: ${noteList || "none"}
...
function registerIpcHandlers() {
  ...
  electron.ipcMain.handle("ai:chat", async (_event, messages, query) => {
    ...
    console.log("[AI] Parsing intent from:", lastMessage.content);
    const intent = await parseIntent(lastMessage.content);
    ...
    if (intent.action === "question" || intent.action === "unknown") {
      ...
      const response = await aiService.chat(messages, context, false);
  ...
  electron.ipcMain.handle("ai:execute", async (_event, actions) => {
    ...
    for (const action of actions) {
      ...
      try {
        if (action.name === "move_note_to_folder" && action.arguments.noteId && action.arguments.folderId) {
          await noteRepository.update(action.arguments.noteId, { folderId: action.arguments.folderId });
          results.push(`Moved note to folder`);
        } else if (action.name === "create_folder" && action.arguments.name) {
          await folderRepository.create({ name: action.arguments.name });
          results.push(`Created folder "${action.arguments.name}"`);
        } else if (action.name === "delete_folder" && action.arguments.folderId) {
          await folderRepository.delete(action.arguments.folderId);
          results.push(`Deleted folder`);
        } else {
          results.push(`Executed ${action.name}`);
        }
      } catch (error) {
