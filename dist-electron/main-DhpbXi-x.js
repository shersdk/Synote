...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await Promise.resolve().then(() => require("./fileFromPath-BuCKnSn_.js"));
...
class AIService {
   ...
   */
  async chat(messages, context = [], enableTools = true, model = "openai/gpt-4o-mini") {
    var _a3;
