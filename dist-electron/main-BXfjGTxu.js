...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await Promise.resolve().then(() => require("./fileFromPath-BUuT6J-X.js"));
...
function registerIpcHandlers() {
  ...
  electron.ipcMain.handle("ai:chat", async (_event, messages, query) => {
    ...
    const notesSummary = allNotes.map((n2) => `- "${n2.title}" (ID: ${n2.id}, folder: ${n2.folderId || "none"})`).join("\n");
...
CURRENT FOLDERS:
${foldersSummary || "(No folders yet)"}

IMPORTANT: Use the exact IDs shown above when calling tools. Do NOT call list_notes or list_folders - you already have this information.
`;
...
${existingDataContext}`
    ...
    let allActions = [];
    let currentMsgs = [...enhancedMessages];
    for (let pass = 0; pass < 2; pass++) {
      const response = await aiService.chat(currentMsgs, context);
      if (!response.toolCalls || response.toolCalls.length === 0) {
        if (allActions.length > 0) {
          return {
            content: response.content || `Done! ${allActions.join(". ")}`,
            actions: allActions
          };
        }
        return {
      ...
      for (const toolCall of response.toolCalls) {
        console.log(`[AI] Executing tool: ${toolCall.name}`, toolCall.arguments);
        const result = await executeToolCall(toolCall.name, toolCall.arguments);
        if (result.success && ((_a3 = result.data) == null ? void 0 : _a3.message)) {
          passResults.push(result.data.message);
          allActions.push(result.data.message);
          if (toolCall.name === "create_folder" && result.data.folder) {
            createdFolders.push({ name: result.data.folder.name, id: result.data.folder.id });
          }
        } else if (!result.success) {
          passResults.push(`Failed: ${result.error}`);
          allActions.push(`Failed: ${result.error}`);
        }
      }
      if (createdFolders.length > 0 && pass === 0) {
        const newFolderInfo = createdFolders.map((f2) => `NEW FOLDER: "${f2.name}" (ID: ${f2.id})`).join("\n");
        currentMsgs = [
          ...enhancedMessages,
          { role: "assistant", content: `Created folders. Results:
${passResults.join("\n")}` },
          { role: "user", content: `Now complete the remaining actions. ${newFolderInfo}
Move the notes using the NEW folder IDs.` }
        ];
        continue;
      }
      break;
    ...
    return {
      content: `Completed ${allActions.length} action(s)!`,
      actions: allActions
    };
  });
