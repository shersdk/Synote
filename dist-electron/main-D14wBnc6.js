...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await Promise.resolve().then(() => require("./fileFromPath-C9sHxLAP.js"));
...
function registerIpcHandlers() {
  ...
  electron.ipcMain.handle("ai:chat", async (_event, messages, query) => {
    var _a3;
    let context = await embeddingService.searchSimilar(query, 3);
    if (context.length === 0) {
      const allNotes = await noteRepository.findAll();
      const queryWords = query.toLowerCase().split(/\s+/);
      for (const note of allNotes.slice(0, 10)) {
        const content = await fileService.readNote(note.filePath);
        const noteWords = (note.title + " " + content).toLowerCase();
        const matchCount = queryWords.filter((w2) => noteWords.includes(w2)).length;
        if (matchCount > 0) {
          context.push({
            id: note.id,
            title: note.title,
      ...
      context.sort((a2, b2) => (b2.similarity || 0) - (a2.similarity || 0));
      context = context.slice(0, 3);
    }
    const MAX_TOOL_ITERATIONS = 5;
    let currentMessages = [...messages];
    let toolActions = [];
    for (let i2 = 0; i2 < MAX_TOOL_ITERATIONS; i2++) {
      const response = await aiService.chat(currentMessages, context);
      ...
      currentMessages.push({
        role: "assistant",
        content: response.content || `Executing ${toolResults.length} action(s)...`
      });
      const actionSummary = toolResults.map((r2) => {
        var _a4;
        const parsed = JSON.parse(r2.result);
        return parsed.success ? (_a4 = parsed.data) == null ? void 0 : _a4.message : `Error: ${parsed.error}`;
      }).join("\n");
      currentMessages.push({
        role: "user",
...
Please summarize what you did and if there's anything else needed.`
      });
    }
    return {
      content: "I completed several actions. " + toolActions.join(". "),
      actions: toolActions
    };
  });
