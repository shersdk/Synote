...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await Promise.resolve().then(() => require("./fileFromPath-B2yrEQE6.js"));
...
class AIService {
   ...
   */
  async chat(messages, context = [], enableTools = true, model = "anthropic/claude-3.5-sonnet") {
    var _a3;
    const client = await this.getClient();
    let systemPrompt = `You are a helpful AI assistant integrated into Synote, a note-taking application. 
You help users with their notes, answer questions, and assist with organizing.
Be concise and friendly.

TOOL USAGE RULES:
1. The user's message includes [Context] with ALL their notes and folders with IDs
2. NEVER call list_notes or list_folders - you already have this information
3. When moving notes: use move_note_to_folder with the exact IDs from context
4. When creating folders: only if the folder doesn't exist in context
5. Match note/folder names case-insensitively 
6. Execute the minimal number of tool calls needed

Example: If user says "put Test note in Work folder" and context shows:
- Note "Test" (ID: abc123)
- Folder "Work" (ID: def456)
Then call: move_note_to_folder(noteId: "abc123", folderId: "def456")`;
    if (context.length > 0) {
      systemPrompt += `
