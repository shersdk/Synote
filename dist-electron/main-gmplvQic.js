...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await Promise.resolve().then(() => require("./fileFromPath-Kd0jAygV.js"));
...
class AIService {
  ...
  async chat(messages, context = [], enableTools = true, model) {
    ...
    let systemPrompt = `You are an AI assistant for Synote, a note-taking app. You EXECUTE actions immediately using tools.

CRITICAL RULES:
1. EXECUTE TOOLS IMMEDIATELY when user asks for an action - don't just describe what you would do
2. The user message contains [Context] with ALL notes and folders + their exact IDs
3. NEVER call list_notes or list_folders - you already have this data
4. Call MULTIPLE tools in ONE response when needed (e.g., create folder THEN move notes)
5. Use exact IDs from context for move_note_to_folder

When user says "move all notes to X folder":
1. If folder X doesn't exist in context → call create_folder
2. For EACH note → call move_note_to_folder with that note's ID

Example for "move Test and Doc to Work folder":
- If Work folder exists (ID: folder123), call:
  move_note_to_folder(noteId: "note1", folderId: "folder123")
  move_note_to_folder(noteId: "note2", folderId: "folder123")`;
    if (context.length > 0) {
      systemPrompt += `
  ...
  try {
    switch (toolName) {
      ...
      case "move_note_to_folder": {
        ...
        await noteRepository.update(args.noteId, {
          folderId: args.folderId === "null" || args.folderId === null ? null : args.folderId
        });
        const folder = args.folderId ? await folderRepository.findById(args.folderId) : null;
        return {
          success: true,
          data: {
...
FOLDERS:
${foldersSummary || "(No folders)"}

CRITICAL: If a folder with matching name EXISTS above, USE ITS ID. Do NOT create duplicates.
Match names case-insensitively: "Speed12" = "speed12" = "SPEED12"
`;
