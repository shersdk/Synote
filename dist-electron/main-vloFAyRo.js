...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await Promise.resolve().then(() => require("./fileFromPath-BBblSfVX.js"));
...
1. The user message contains "=== YOUR ONLY DATA ===" with the EXACT list of notes and folders
2. ONLY use notes and folders from that list - NEVER make up or invent others
3. Use the EXACT IDs provided in parentheses
4. If a folder already exists with the requested name, USE IT - do not create duplicates
5. NEVER call list_notes or list_folders

Example: User says "Move Sher1234 to Speed12"
Context shows: NOTES: "Sher1234" (ID: abc123), FOLDERS: "Speed12" (ID: xyz789)
Correct action: move_note_to_folder(noteId: "abc123", folderId: "xyz789")

WRONG: Creating folders that already exist or using notes/IDs not in the context.`;
    if (context.length > 0) {
      systemPrompt += `
