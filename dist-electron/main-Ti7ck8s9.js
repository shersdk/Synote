...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await Promise.resolve().then(() => require("./fileFromPath-KqIkJjWL.js"));
...
You help users with their notes, answer questions, and assist with writing and research.
Be concise, helpful, and friendly.

You have access to tools to manage notes and folders. When the user asks you to:
- Create folders or organize notes, use the appropriate tools
- Find, search, or list notes, use the list_notes or search_notes tools
- Move notes to folders, first list folders and notes to get the IDs, then use move_note_to_folder

Always confirm actions you've taken and summarize the results.`;
    if (context.length > 0) {
      systemPrompt += `
