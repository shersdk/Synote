...
var _a2, _b, _c, _d, _e2, _f, _g, _h, _i, _j, _k, _l, _m, _n, _o, _p, _q, _r2, _s, _t2, _u, _v, _w, _x, _y, _z, _A, _B, _C, _D, _E, _F, _G, _H, _I, _J, _K, _L, _M, _N, _O, _P, _Q, _R, _S, _T, _U, _V, _W, _X, _Y, _Z, __, _$, _aa, _ba, _ca, _da, _ea, _fa, _ga, _ha, _ia, _ja, _ka, _la, _ma, _na, _oa, _pa, _qa, _ra, _sa, _ta, _ua, _va, _wa, _xa, _ya, _za, _Aa, _Ba, _Ca, _Da, _Ea, _Fa, _Ga, _Ha, _Ia, _Ja, _Ka, _La, _Ma, _Na, _Oa, _Pa, _Qa, _Ra, _Sa, _Ta, _Ua, _Va, _Wa, _Xa, _Ya, _Za, __a, _$a, _ab, _bb, _cb, _db, _eb, _fb, _gb, _hb, _ib, _jb, _kb, _lb, _mb, _nb, _ob, _pb, _qb, _rb, _sb, _tb, _ub, _vb, _wb, _xb, _yb, _zb, _Ab, _Bb, _Cb, _Db, _Eb, _Fb, _Gb, _Hb, _Ib, _Jb, _Kb, _Lb, _Mb, _Nb, _Ob, _Pb, _Qb, _Rb;
...
const require$$0 = require("punycode");
...
_m = entityKind, _l = TableName, _k = OriginalName, _j = Schema, _i = Columns, _h = ExtraConfigColumns, _g = BaseName, _f = IsAlias, _e2 = IsDrizzleTable, _d = ExtraConfigBuilder;
class Table {
...
class WithSubquery extends (_r2 = Subquery, _q = entityKind, _r2) {
}
    ...
    return mergeQueries(chunks.map((chunk) => {
      ...
      if (is(chunk, Column)) {
        ...
        if (_config.invokeSource === "indexes") {
          return { sql: escapeName2(columnName), params: [] };
        }
        const schemaName = chunk.table[Table.Symbol.Schema];
        return {
          sql: chunk.table[IsAlias] || schemaName === void 0 ? escapeName2(chunk.table[Table.Symbol.Name]) + "." + escapeName2(columnName) : escapeName2(schemaName) + "." + escapeName2(chunk.table[Table.Symbol.Name]) + "." + escapeName2(columnName),
          params: []
        };
      }
      if (is(chunk, View)) {
...
class SQLiteDialect {
  ...
  buildUpdateSet(table, set) {
    ...
    const columnNames = Object.keys(tableColumns).filter(
      (colName) => {
        var _a3;
        return set[colName] !== void 0 || ((_a3 = tableColumns[colName]) == null ? void 0 : _a3.onUpdateFn) !== void 0;
      }
    );
  ...
  buildUpdateQuery({ table, set, where, returning, withList, joins, from, limit: limit2, orderBy }) {
    const withSql = this.buildWithCTE(withList);
...
const notes = sqliteTable("notes", {
  ...
  // Path to .md file relative to notes directory
  folderId: text("folder_id").references(() => folders.id, { onDelete: "set null" }),
  // For AI/RAG features
  embedding: text("embedding"),
  // JSON string of float32 array for vector search
  summary: text("summary"),
  // AI-generated summary for better search
  // Timestamps
  createdAt: integer("created_at", { mode: "timestamp" }).default(sql`(unixepoch())`).notNull(),
  updatedAt: integer("updated_at", { mode: "timestamp" }).default(sql`(unixepoch())`).notNull()
});
...
class NoteRepository extends BaseRepository {
  async findById(id) {
    ...
    const result = await db2.select().from(notes).where(eq(notes.id, id)).limit(1);
    return result[0] ?? null;
  }
  async findAll() {
    const db2 = getDatabase();
    return db2.select().from(notes).orderBy(notes.updatedAt);
  }
  async findByFolderId(folderId) {
    const db2 = getDatabase();
    if (folderId === null) {
      return db2.select().from(notes).where(eq(notes.folderId, folderId));
    }
    return db2.select().from(notes).where(eq(notes.folderId, folderId));
  }
  async create(data) {
    ...
    await db2.insert(notes).values(newNote);
    return { ...newNote, embedding: null, summary: null };
  }
  async update(id, data) {
    const db2 = getDatabase();
    const now = /* @__PURE__ */ new Date();
    await db2.update(notes).set({ ...data, updatedAt: now }).where(eq(notes.id, id));
  ...
  async findAllWithEmbeddings() {
    const db2 = getDatabase();
    return db2.select().from(notes).where(eq(notes.embedding, notes.embedding));
  }
}
...
class FolderRepository extends BaseRepository {
  async findById(id) {
    const db2 = getDatabase();
    const result = await db2.select().from(folders).where(eq(folders.id, id)).limit(1);
    return result[0] ?? null;
  }
  async findAll() {
    const db2 = getDatabase();
    return db2.select().from(folders).orderBy(folders.name);
  }
  /**
  ...
  async findRootFolders() {
    const db2 = getDatabase();
    return db2.select().from(folders).where(isNull(folders.parentId)).orderBy(folders.name);
  }
  /**
  ...
  async findByParentId(parentId) {
    const db2 = getDatabase();
    return db2.select().from(folders).where(eq(folders.parentId, parentId)).orderBy(folders.name);
  }
  async create(data) {
    const db2 = getDatabase();
    const id = v4();
    ...
    await db2.insert(folders).values(newFolder);
    return newFolder;
  }
  async update(id, data) {
    const db2 = getDatabase();
    const now = /* @__PURE__ */ new Date();
    await db2.update(folders).set({ ...data, updatedAt: now }).where(eq(folders.id, id));
    return this.findById(id);
  }
  async delete(id) {
    const db2 = getDatabase();
    await db2.delete(folders).where(eq(folders.id, id));
    return true;
  }
  /**
  ...
  buildTree(folders2, parentId) {
    return folders2.filter((f2) => f2.parentId === parentId).map((folder) => ({
      ...folder,
      children: this.buildTree(folders2, folder.id)
    }));
  }
}
...
class FileService {
  constructor() {
    ...
    this.notesDir = path.join(electron.app.getPath("documents"), "Synote Notes");
    this.ensureNotesDir();
  }
  ensureNotesDir() {
   ...
   */
  generateFilePath(title) {
    const sanitized = title.replace(/[^a-zA-Z0-9\s-]/g, "").replace(/\s+/g, "-").toLowerCase().slice(0, 50);
  ...
  async listNotes() {
    try {
      ...
      return files.filter((f2) => f2.endsWith(".md"));
    } catch (error) {
   ...
   */
  htmlToMarkdown(html) {
    let md = html.replace(/<h1[^>]*>(.*?)<\/h1>/gi, "# $1\n\n").replace(/<h2[^>]*>(.*?)<\/h2>/gi, "## $1\n\n").replace(/<h3[^>]*>(.*?)<\/h3>/gi, "### $1\n\n").replace(/<p[^>]*>(.*?)<\/p>/gi, "$1\n\n").replace(/<strong[^>]*>(.*?)<\/strong>/gi, "**$1**").replace(/<em[^>]*>(.*?)<\/em>/gi, "*$1*").replace(/<code[^>]*>(.*?)<\/code>/gi, "`$1`").replace(/<pre[^>]*><code[^>]*>(.*?)<\/code><\/pre>/gis, "```\n$1\n```\n\n").replace(/<li[^>]*>(.*?)<\/li>/gi, "- $1\n").replace(/<\/?[uo]l[^>]*>/gi, "\n").replace(/<blockquote[^>]*>(.*?)<\/blockquote>/gis, (_2, content) => {
   ...
   */
  markdownToHtml(md) {
    let html = md.replace(/^### (.*$)/gim, "<h3>$1</h3>").replace(/^## (.*$)/gim, "<h2>$1</h2>").replace(/^# (.*$)/gim, "<h1>$1</h1>").replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>").replace(/\*(.+?)\*/g, "<em>$1</em>").replace(/```([^`]+)```/g, "<pre><code>$1</code></pre>").replace(/`([^`]+)`/g, "<code>$1</code>").replace(/^- (.*$)/gim, "<li>$1</li>").replace(/^> (.*$)/gim, "<blockquote>$1</blockquote>").replace(/^---$/gim, "<hr>").replace(/\n\n/g, "</p><p>").replace(/^(.+)$/gm, (match) => {
...
let Blob$1 = class Blob {
  ...
  slice() {
    ...
    const blob2 = new Blob([], { type: arguments[2] });
...
async function* consumeNodeBlob(blob2) {
  let position = 0;
  while (position !== blob2.size) {
    const chunk = blob2.slice(position, Math.min(blob2.size, position + CHUNK_SIZE));
...
const deprecateConstructorEntries = require$$0$1.deprecate(() => {
...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await Promise.resolve().then(() => require("./fileFromPath-CBGSz46p.js"));
...
async function getMultipartRequestOptions(form, opts) {
  ...
  const readable = node_stream.Readable.from(encoder);
...
async function toFile(value, name, options) {
  ...
  if (isResponseLike(value)) {
    ...
    const data = isBlobLike(blob2) ? [await blob2.arrayBuffer()] : [blob2];
...
const readEnv = (env) => {
  ...
  if (typeof Deno !== "undefined") {
    return (_e3 = (_d2 = (_c2 = Deno.env) == null ? void 0 : _c2.get) == null ? void 0 : _d2.call(_c2, env)) == null ? void 0 : _e3.trim();
  }
  return void 0;
};
...
function debug(action, ...args) {
  var _a3;
  if (typeof process !== "undefined" && ((_a3 = process.env) == null ? void 0 : _a3["DEBUG"]) === "true") {
...
const getHeader = (headers, header) => {
  ...
  if (isHeadersProtocol(headers)) {
    const intercapsHeader = ((_a3 = header[0]) == null ? void 0 : _a3.toUpperCase()) + header.substring(1).replace(/([^\w])(\w)/g, (_m2, g1, g2) => g1 + g2.toUpperCase());
...
class ChatCompletionStream extends AbstractChatCompletionRunner {
  ...
  [(_ChatCompletionStream_params = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_choiceEventStates = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_currentChatCompletionSnapshot = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_instances = /* @__PURE__ */ new WeakSet(), _ChatCompletionStream_beginRequest = function _ChatCompletionStream_beginRequest2() {
    ...
    for (const choice of chunk.choices) {
      ...
      if (((_e3 = choice.logprobs) == null ? void 0 : _e3.content) != null && ((_f2 = choiceSnapshot.message) == null ? void 0 : _f2.role) === "assistant") {
      ...
      if (((_i2 = choice.logprobs) == null ? void 0 : _i2.refusal) != null && ((_j2 = choiceSnapshot.message) == null ? void 0 : _j2.role) === "assistant") {
        this._emit("logprobs.refusal.delta", {
          ...
          snapshot: ((_l2 = choiceSnapshot.logprobs) == null ? void 0 : _l2.refusal) ?? []
        });
      }
      ...
      for (const toolCallDelta of choice.delta.tool_calls ?? []) {
        const toolCallSnapshot = (_m2 = choiceSnapshot.message.tool_calls) == null ? void 0 : _m2[toolCallDelta.index];
        if (!(toolCallSnapshot == null ? void 0 : toolCallSnapshot.type)) {
          continue;
        }
        if ((toolCallSnapshot == null ? void 0 : toolCallSnapshot.type) === "function") {
          this._emit("tool_calls.function.arguments.delta", {
            name: (_n2 = toolCallSnapshot.function) == null ? void 0 : _n2.name,
            index: toolCallDelta.index,
            arguments: toolCallSnapshot.function.arguments,
            parsed_arguments: toolCallSnapshot.function.parsed_arguments,
            arguments_delta: ((_o2 = toolCallDelta.function) == null ? void 0 : _o2.arguments) ?? ""
          });
        } else {
...
const API_KEY_FILE$1 = path.join(electron.app.getPath("userData"), ".api-key");
const MODEL_FILE = path.join(electron.app.getPath("userData"), ".ai-model");
const DEFAULT_MODEL = "amazon/nova-2-lite-v1:free";
class AIService {
  constructor() {
    __publicField(this, "client", null);
    __publicField(this, "apiKey", null);
    __publicField(this, "modelId", null);
  }
  /**
  ...
  async setApiKey(key) {
    ...
    await fs.promises.writeFile(API_KEY_FILE$1, encrypted);
    this.apiKey = key;
    this.client = null;
  }
  /**
  ...
  async hasApiKey() {
    try {
      await fs.promises.access(API_KEY_FILE$1);
      return true;
    } catch {
      return false;
    }
  ...
  async deleteApiKey() {
    try {
      await fs.promises.unlink(API_KEY_FILE$1);
      this.apiKey = null;
      this.client = null;
    } catch {
    }
  ...
  async getClient() {
    ...
    if (!this.apiKey) {
      try {
        const encrypted = await fs.promises.readFile(API_KEY_FILE$1);
        this.apiKey = electron.safeStorage.decryptString(encrypted);
      } catch {
   ...
   */
  async chat(messages, context = [], enableTools = true, model) {
    var _a3;
...
`;
    ...
    try {
      const { AI_TOOLS } = await Promise.resolve().then(() => require("./ai-tools-D1eu0jEI.js"));
      ...
      const message = (_a3 = completion.choices[0]) == null ? void 0 : _a3.message;
      if (!message) {
   ...
   */
  async continueWithToolResults(messages, toolResults, context = [], model) {
    var _a3;
    const client = await this.getClient();
    const useModel = model || await this.getModel();
    const fullMessages = [
    ...
    try {
      ...
      const message = (_a3 = completion.choices[0]) == null ? void 0 : _a3.message;
      return { content: (message == null ? void 0 : message.content) || "Action completed." };
    } catch (error) {
  ...
  async generateEmbedding(text2) {
    ...
    try {
      ...
      return ((_a3 = response.data[0]) == null ? void 0 : _a3.embedding) || [];
    } catch (error) {
  ...
  cosineSimilarity(a2, b2) {
    if (a2.length !== b2.length || a2.length === 0) return 0;
    let dotProduct = 0;
    let normA = 0;
    let normB = 0;
...
class EmbeddingService {
   ...
   */
  async embedNote(noteId) {
    const note = await noteRepository.findById(noteId);
    if (!note) return;
    const content = await fileService.readNote(note.filePath);
    if (!content.trim()) return;
    const text2 = `${note.title}

${content}`;
    const embedding = await aiService.generateEmbedding(text2);
   ...
   */
  async searchSimilar(query, limit2 = 5) {
    const queryEmbedding = await aiService.generateEmbedding(query);
    if (queryEmbedding.length === 0) {
      return [];
    }
    const notes2 = await noteRepository.findAll();
    const results = [];
    ...
    return results.sort((a2, b2) => (b2.similarity || 0) - (a2.similarity || 0)).slice(0, limit2);
  }
  /**
    ...
    switch (toolName) {
      case "create_folder": {
        ...
        const existing = existingFolders.find(
          (f2) => f2.name.toLowerCase() === args.name.toLowerCase()
        );
        if (existing) {
      ...
      case "move_note_to_folder": {
        ...
        if (!note) {
          ...
          const foundNote = allNotes.find((n2) => n2.title.toLowerCase() === args.noteId.toLowerCase());
          if (!foundNote) {
        ...
        if (targetFolderId) {
          ...
          if (!folder) {
            ...
            folder = allFolders.find(
              (f2) => f2.name.toLowerCase() === targetFolderId.toLowerCase() || f2.name.toLowerCase() === args.folderId.toLowerCase()
            );
            if (folder) {
        ...
        return {
          success: true,
          data: {
            message: folder ? `Moved note "${note.title}" to folder "${folder.name}"` : `Moved note "${note.title}" to root level`
          }
        };
      }
      case "create_note": {
      ...
      case "list_notes": {
        let notes2;
        if (args.folderId) {
          notes2 = await noteRepository.findByFolderId(args.folderId);
        } else {
          notes2 = await noteRepository.findAll();
        }
        return {
          success: true,
          data: {
      ...
      case "list_folders": {
        const folders2 = await folderRepository.findAll();
        return {
          success: true,
          data: {
            folders: folders2.map((f2) => ({
              id: f2.id,
              name: f2.name,
              parentId: f2.parentId
            })),
            count: folders2.length
          }
        };
      }
      case "rename_note": {
      ...
      case "delete_folder": {
        ...
        const notes2 = await noteRepository.findByFolderId(args.folderId);
        for (const note of notes2) {
        ...
        return {
          success: true,
          data: { message: `Deleted folder "${folder.name}". ${notes2.length} notes moved to root.` }
        };
      }
      case "search_notes": {
        ...
        if (results.length > 0) {
          return {
            success: true,
            data: {
              notes: results.map((r2) => ({
                id: r2.id,
                title: r2.title,
                similarity: r2.similarity,
                preview: r2.content.slice(0, 200)
              })),
...
function findBestMatch(query, items, maxDistance = 3) {
  ...
  for (const item of items) {
    ...
    if (itemName.includes(queryLower) || queryLower.includes(itemName)) {
      const distance2 = Math.abs(itemName.length - queryLower.length);
...
async function parseCommand(message) {
  ...
  const notes2 = await noteRepository.findAll();
  const folders2 = await folderRepository.findAll();
  const movePattern = /(?:move|put|add)\s+(.+?)\s+(?:to|in|into)\s+(.+?)(?:\s+folder)?$/i;
  const moveMatch = message.match(movePattern);
  if (moveMatch) {
    const noteName = moveMatch[1].trim();
    const folderName = moveMatch[2].trim();
    const note = findBestMatch(noteName, notes2.map((n2) => ({ id: n2.id, title: n2.title })));
    const folder = findBestMatch(folderName, folders2.map((f2) => ({ id: f2.id, name: f2.name })));
    if (note && folder) {
      return {
        handled: true,
        command: {
  ...
  const createFolderPattern = /(?:create|make|new)\s+(?:a\s+)?folder\s+(?:called\s+|named\s+)?(.+)$/i;
  const createFolderMatch = message.match(createFolderPattern);
  if (createFolderMatch) {
    const folderName = createFolderMatch[1].trim().replace(/["']/g, "");
    const existing = findBestMatch(folderName, folders2.map((f2) => ({ id: f2.id, name: f2.name })));
    if (existing && existing.name.toLowerCase() === folderName.toLowerCase()) {
      return {
        handled: true,
        command: {
          type: "create_folder",
        ...
        error: `Folder "${existing.name}" already exists`
      };
    }
    return {
      handled: true,
  ...
  const deleteFolderPattern = /(?:delete|remove)\s+(?:the\s+)?folder\s+(.+)$/i;
  const deleteFolderMatch = message.match(deleteFolderPattern);
  if (deleteFolderMatch) {
    const folderName = deleteFolderMatch[1].trim().replace(/["']/g, "");
    const folder = findBestMatch(folderName, folders2.map((f2) => ({ id: f2.id, name: f2.name })));
    if (folder) {
      return {
        handled: true,
  ...
  const listPattern = /(?:list|show|what|all)\s+(?:my\s+)?(?:notes|folders)/i;
...
function formatCommandDescription(cmd) {
  switch (cmd.type) {
    case "move_note":
      if (cmd.folderId) {
        return `Move "${cmd.noteName}" to folder "${cmd.folderName}"`;
      }
      return `Move "${cmd.noteName}" (folder not found)`;
    case "create_folder":
      return `Create folder "${cmd.folderName}"`;
    case "delete_folder":
      return `Delete folder "${cmd.folderName}"`;
    case "rename_note":
      return `Rename "${cmd.noteName}" to "${cmd.newName}"`;
    case "rename_folder":
      return `Rename folder "${cmd.folderName}" to "${cmd.newName}"`;
    case "list":
      return "List notes and folders";
    default:
...
function registerIpcHandlers() {
  ...
  electron.ipcMain.handle("notes:create", async (_event, data) => {
  ...
  electron.ipcMain.handle("notes:update", async (_event, id, data) => {
    const note = await noteRepository.findById(id);
    if (!note) return null;
    if (data.content) {
  ...
  electron.ipcMain.handle("folders:list", async () => {
    return folderRepository.findAll();
  });
  electron.ipcMain.handle("folders:tree", async () => {
    return folderRepository.getTree();
  });
  electron.ipcMain.handle("folders:create", async (_event, data) => {
    return folderRepository.create(data);
  });
  electron.ipcMain.handle("folders:update", async (_event, id, data) => {
    return folderRepository.update(id, data);
  });
  electron.ipcMain.handle("folders:delete", async (_event, id) => {
    return folderRepository.delete(id);
  });
  electron.ipcMain.handle("ai:chat", async (_event, messages, query) => {
    const lastMessage = messages[messages.length - 1];
    if ((lastMessage == null ? void 0 : lastMessage.role) === "user") {
      const parseResult = await parseCommand(lastMessage.content);
      if (parseResult.handled && parseResult.command) {
        console.log("[Command Parser] Recognized:", parseResult.command.type);
        const cmd = parseResult.command;
        let pendingAction = null;
        if (cmd.type === "move_note" && cmd.noteId && cmd.folderId) {
          pendingAction = {
            id: "cmd-" + Date.now(),
            name: "move_note_to_folder",
            arguments: { noteId: cmd.noteId, folderId: cmd.folderId },
            description: formatCommandDescription(cmd)
          };
        } else if (cmd.type === "create_folder" && cmd.folderName && !parseResult.error) {
          pendingAction = {
            id: "cmd-" + Date.now(),
            name: "create_folder",
            arguments: { name: cmd.folderName },
            description: formatCommandDescription(cmd)
          };
        } else if (cmd.type === "delete_folder" && cmd.folderId) {
          pendingAction = {
            id: "cmd-" + Date.now(),
            name: "delete_folder",
            arguments: { folderId: cmd.folderId },
            description: formatCommandDescription(cmd)
          };
        }
        if (pendingAction) {
    ...
    console.log("[AI] Falling back to AI for complex query");
    const allNotes = await noteRepository.findAll();
    const allFolders = await folderRepository.findAll();
    let context = [];
    ...
    const notesSummary = allNotes.map((n2) => `"${n2.title}" (ID: ${n2.id})`).join(", ");
    const foldersSummary = allFolders.map((f2) => `"${f2.name}" (ID: ${f2.id})`).join(", ");
    const existingDataContext = `
=== YOUR ONLY DATA ===
NOTES: ${notesSummary || "NONE"}
FOLDERS: ${foldersSummary || "NONE"}
======================

STRICT RULES:
1. ONLY use notes and folders from the list above - do NOT invent or make up any
2. Use the EXACT IDs provided - no other IDs exist
3. If folder "${allFolders.map((f2) => f2.name).join("/")}" exists, use its ID
4. Do NOT create folders that already exist
`;
    console.log("[AI] Context:", existingDataContext);
    const enhancedMessages = [...messages];
    const lastMsg = enhancedMessages[enhancedMessages.length - 1];
    if ((lastMsg == null ? void 0 : lastMsg.role) === "user") {
...
${existingDataContext}`
    ...
    const response = await aiService.chat(enhancedMessages, context);
    if (!response.toolCalls || response.toolCalls.length === 0) {
      return { content: response.content, pendingActions: null };
    }
    const pendingActions = response.toolCalls.map((tc) => ({
      id: tc.id,
      name: tc.name,
      arguments: tc.arguments,
      description: describeAction(tc.name, tc.arguments, allNotes, allFolders)
    }));
    return {
      content: response.content || "I want to perform these actions:",
      pendingActions
    };
  });
  electron.ipcMain.handle("ai:execute", async (_event, actions) => {
    var _a3;
    const results = [];
    for (const action of actions) {
      console.log(`[AI] Executing approved: ${action.name}`, action.arguments);
      const result = await executeToolCall(action.name, action.arguments);
      if (result.success && ((_a3 = result.data) == null ? void 0 : _a3.message)) {
        results.push(result.data.message);
      } else if (!result.success) {
        results.push(`Failed: ${result.error}`);
      }
    }
    return { actions: results };
  });
  electron.ipcMain.handle("ai:generateEmbedding", async (_event, text2) => {
    return aiService.generateEmbedding(text2);
  });
  electron.ipcMain.handle("ai:embedAllNotes", async () => {
    await embeddingService.embedAllNotes();
    return true;
  });
  electron.ipcMain.handle("settings:getNotesDir", () => {
    return fileService.getNotesDir();
  });
  electron.ipcMain.handle("settings:getModel", () => {
    return aiService.getModel();
  });
  electron.ipcMain.handle("settings:setModel", async (_event, model) => {
    await aiService.setModel(model);
    return true;
  });
}
function describeAction(name, args, notes2, folders2) {
  switch (name) {
    case "create_folder":
      return `Create folder "${args.name}"`;
    case "move_note_to_folder": {
      const note = notes2.find((n2) => n2.id === args.noteId);
      const folder = folders2.find((f2) => f2.id === args.folderId);
      return `Move "${(note == null ? void 0 : note.title) || "note"}" → "${(folder == null ? void 0 : folder.name) || "folder"}"`;
    }
    case "create_note":
      return `Create note "${args.title}"`;
    case "rename_note": {
      const n2 = notes2.find((x2) => x2.id === args.noteId);
      return `Rename "${(n2 == null ? void 0 : n2.title) || "note"}" → "${args.newTitle}"`;
    }
    case "rename_folder": {
      const f2 = folders2.find((x2) => x2.id === args.folderId);
      return `Rename folder "${(f2 == null ? void 0 : f2.name) || "folder"}" → "${args.newName}"`;
    }
    case "delete_folder": {
      const f2 = folders2.find((x2) => x2.id === args.folderId);
      return `Delete folder "${(f2 == null ? void 0 : f2.name) || "folder"}"`;
    }
    default:
      return `${name}`;
  }
...
class FileWatcherService {
   ...
   */
  notifyRenderer(eventType, filename) {
    const windows = electron.BrowserWindow.getAllWindows();
    for (const window2 of windows) {
      window2.webContents.send("file:changed", {
        type: eventType,
        filename,
...
const API_KEY_FILE = path.join(electron.app.getPath("userData"), ".api-key");
electron.ipcMain.handle("api-key:set", async (_event, key) => {
  if (!electron.safeStorage.isEncryptionAvailable()) {
    throw new Error("Encryption not available on this system");
  }
  const encrypted = electron.safeStorage.encryptString(key);
...
electron.ipcMain.handle("api-key:has", async () => {
  try {
...
electron.ipcMain.handle("api-key:get", async () => {
  try {
...
electron.ipcMain.handle("api-key:delete", async () => {
  try {
