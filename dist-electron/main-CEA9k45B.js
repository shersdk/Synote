...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await Promise.resolve().then(() => require("./fileFromPath-juwuYX-6.js"));
...
class AIService {
   ...
   */
  async chat(messages, context = [], enableTools = true, model = "arcee-ai/trinity-mini:free") {
    var _a3;
...
`;
   ...
   */
  async continueWithToolResults(messages, toolResults, context = [], model = "anthropic/claude-3.5-sonnet") {
    var _a3;
    const client = await this.getClient();
    const fullMessages = [
