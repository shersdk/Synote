...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await Promise.resolve().then(() => require("./fileFromPath--kGei-wC.js"));
...
`;
    ...
    try {
      const { AI_TOOLS } = await Promise.resolve().then(() => require("./ai-tools-s-1BFn7T.js"));
  ...
  switch (intent.action) {
    ...
    case "list": {
      return {
        ...
        action: {
          type: "list_notes",
          // Handled by tool-executor as 'list_notes' (or 'list_folders'?)
  ...
  electron.ipcMain.handle("ai:execute", async (_event, actions) => {
    ...
    for (const action of actions) {
      ...
      try {
        ...
        if (result.success) {
          if ((_a3 = result.data) == null ? void 0 : _a3.message) {
